...
import type { MemoryFeeling as Feeling } from '@/types/api.types';
...
// Component to expose map instance for flyTo
...
function createEmojiIcon(emoji: string, color: string) {
  return L.divIcon({
    html: `
      ...
      ">
        <div style="font-size: 24px; line-height: 24px;">${emoji}</div>
      </div>
    `,
...
function createAvatarIcon(avatarUrl: string, feeling: string | undefined, color: string) {
  const feelingHtml = feeling 
    ? `<div style="
        ...
        z-index: 10;
      ">
        ${FEELING_EMOJIS[feeling as Feeling] || '📍'}
      </div>` 
    : '';
      ...
      () => ({
        flyTo: (region: MapRegion, duration = 2000) => {
          if (mapInstanceRef.current) {
            const zoom = Math.round(Math.log2(180 / region.latitudeDelta));
            ...
            <Marker
              ...
              icon={
                (memory.userId !== currentUserId) || memory.user?.avatarUrl 
                  ? createAvatarIcon(memory.user?.avatarUrl || '', memory.feeling, getMarkerColor(memory))
                  : memory.feeling 
                    ? createEmojiIcon(FEELING_EMOJIS[memory.feeling], getMarkerColor(memory)) 
                    : createColoredIcon(getMarkerColor(memory))
              }
              eventHandlers={{
                click: () => onMemoryPress?.(memory),
              }}
            >
              <Popup autoPan={false}>
                <div style={{ minWidth: 120 }}>
                  <strong>
                    {memory.user?.name ? `${memory.user.name} • ` : ''}
                    {memory.type === 'voice'
                      ? '🎤 Voice'
                      : memory.type === 'photo'
                        ? '📷 Photo'
